...
def do_login():
    ...
    r = requests.post ('https://www.netlab.tkk.fi/mailman/private/theone/', data=data, verify=False)
    return r
...
def lucene_indexing():
    ...
    index_writer = lucene.IndexWriter(dir,analyzer,True,lucene.IndexWriter.MaxFieldLength.LIMITED)
    ...
    for tfile in glob.glob(os.path.join(index_dir,'*.txt')):
        ...
        document.add(Field("title",tfile,Field.Store.YES,Field.Index.ANALYZED))
...
def lucene_search(query):
    ...
    my_query = QueryParser(lucene.Version.LUCENE_CURRENT,"text",lucene_analyzer).parse(query)
    MAX=10
    total_hits = lucene_searcher.search(my_query,MAX)
    
    for hit in total_hits.scoreDocs:
        #dir(hit)
        #print "Hit score:%s  Hit doc:%s  Hit string:%s" % (str(hit.score),str(hit.doc),hit.toString())
...
def main():
    ...
    parse.add_argument("-update",help="Update the archive by downloadig the last monthly entries",action='store_true')
...
if __name__ == "__main__":
    try:
        ...
        msg = "Exception occured in main: "+str(type(exc))+"  args:"+str(exc)+" line:"+str(exc_tb.tb_lineno)
        print msg
